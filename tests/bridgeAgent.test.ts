...
function makePlanArtifactFailureDecision(): PendingUserDecision {
  return {
    ...
    question: 'The heavy agent did not provide a usable plan artifact. What should VibeCodingAssistant do next?',
    rationale: 'VibeCodingAssistant paused before sending an empty plan onward.',
    options: [
...
describe('BridgeAgentService', () => {
  ...
  it('marks internal task status as orphaned when no running job or worker exists', async () => {
    ...
    try {
      ...
      if (turn.kind === 'reply') {
        ...
        expect(text).toContain('可能是上次 VibeCodingAssistant 重启或进程中断');
  ...
  it('adds a project via add_project and makes it active for the current chat', async () => {
    const harness = await makeHarness();
    const projectDir = join(harness.root, 'Sample Project');
    try {
      ...
      harness.assistant.decisions.push({
        ...
        toolCall: {
          name: 'add_project',
          arguments: { targetDir: projectDir, name: 'Sample Project' },
        },
      });
      ...
      expect(turn.activeProjectId).toBe('sample-project');
      expect(turn.messages[0]?.text).toContain('项目已添加');
      expect(harness.config.projects?.some((project) => project.id === 'sample-project')).toBe(true);
      ...
      expect(registry.projects[0]).toMatchObject({ id: 'sample-project', targetDir: projectDir });
    } finally {
  ...
  it('creates the next task in the project added during the same process', async () => {
    ...
    try {
      ...
      harness.assistant.decisions.push({
        ...
        toolCall: {
          name: 'create_task',
          arguments: { title: 'VibeCodingAssistant task', prompt: 'Create a small VibeCodingAssistant task.' },
        },
      });
      const createTurn = await harness.agent.handleMessage({
        chatId: 'control-chat',
        senderOpenId: 'user-open-id',
        text: '创建一个 VibeCodingAssistant task',
        chatKind: 'control',
      ...
      expect(createTurn.projectId).toBe('sample-project');
      expect(createTurn.projectName).toBe('Sample Project');
      const state = await harness.store.loadState(createTurn.taskId);
      expect(state.projectId).toBe('sample-project');
      await expect(readFile(join(projectDir, 'task', createTurn.taskId, 'README.md'), 'utf8'))
        .resolves.toContain('VibeCodingAssistant task');
    } finally {
      await cleanup([harness.root, harness.targetDir, projectDir]);
    }
  });
