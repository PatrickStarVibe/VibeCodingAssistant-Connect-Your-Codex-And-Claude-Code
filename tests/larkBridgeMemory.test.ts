...
describe('chat memory state', () => {
  ...
  it('renames only legacy auto-generated Project Chat names on load', async () => {
    ...
    try {
        ...
        JSON.stringify({
          projectChatsByChatId: {
            'chat-auto': {
              chatId: 'chat-auto',
              projectId: 'assistant',
              name: 'Assistant - [VibeCodingAssistant] #2',
            ...
            'chat-custom': {
              chatId: 'chat-custom',
              projectId: 'assistant',
              name: 'My assistant project room',
              createdAt: '2025-01-01T00:00:00Z',
              updatedAt: '2025-01-01T00:00:00Z',
            },
          },
      ...
      const loaded = await store.load();
      expect(loaded.projectChatsByChatId['chat-auto']?.name).toBe('Assistant - [VibeCodingAssistant] #2');
      expect(loaded.projectChatsByChatId['chat-custom']?.name).toBe('My assistant project room');
      ...
      expect(persisted.projectChatsByChatId['chat-auto']?.name).toBe('Assistant - [VibeCodingAssistant] #2');
      expect(persisted.projectChatsByChatId['chat-custom']?.name).toBe('My assistant project room');
