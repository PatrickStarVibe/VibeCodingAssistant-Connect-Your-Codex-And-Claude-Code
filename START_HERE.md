# VibeCodingAssistant Setup / START HERE

这个文件面向用户自己的 AI coding assistant：下载 VibeCodingAssistant repo 后，用它把本地项目、chat bridge 和模型 profile 配好。真实密钥只放在 `.env.local` 或系统环境变量里，不要提交到 git。
...
- 一个或多个要让 VibeCodingAssistant 管理的本地项目路径，例如 `<YOUR_PROJECT_PATH>`。
...
在 VibeCodingAssistant repo 根目录执行：
...
- `projects[]`：VibeCodingAssistant 可访问的项目列表。每个项目应包含 `id`、`name`、`targetDir`、`docsDir`，可选 `taskRecordRoot` 和 `alwaysRead`。
- `projects[].docsDir`：VibeCodingAssistant repo 内的项目记忆目录，例如 `project-docs/<PROJECT_ID>`；也可以是绝对路径。
...
然后确认 VibeCodingAssistant 能加载项目：
...
## 7. 确认 VibeCodingAssistant 能访问项目目录
...
如果 `projects` 命令能显示正确路径，`agent-prompt-preview` 的 `Target workspace` 指向 `<YOUR_PROJECT_PATH>`，并且后续 artifacts 能正常生成，VibeCodingAssistant 已经能定位并使用用户自己的项目目录。
...
```text
Please configure this VibeCodingAssistant repo for my local AI coding workflow.
