...
Final Review routing is a VibeCodingAssistant-owned decision point. The LLM can advise whether a failed final review should be completed, sent back to implementation, sent back to planning, or surfaced to the user, but it must not be the only safety layer.
...
4. VibeCodingAssistant paused with an internal formatting error instead of giving the user an actionable choice.

From the user's perspective, "VibeCodingAssistant" and "Advisor" are both VibeCodingAssistant. The code should not expose internal role-format errors as normal workflow guidance.

## Rule

VibeCodingAssistant keeps deterministic guardrails after Final Review:
...
- If a user decision is needed but the advisor omits structured A/B/C/D options, VibeCodingAssistant generates safe fallback options.
...
VibeCodingAssistant still limits automatic Final Review follow-up loops. After the automatic follow-up budget is used, repeated implementation blockers pause with explicit options:
...
This cap is VibeCodingAssistant-generated. It should not rely on the routing advisor to produce the options.
...
Then, when VibeCodingAssistant says it has recorded the direction and is ready, reply:
