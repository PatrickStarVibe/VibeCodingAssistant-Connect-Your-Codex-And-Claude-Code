...
The Lark layer is now a thin transport for VibeCodingAssistant.
...
- event id dedupe
- text/file sending
- task chat creation when VibeCodingAssistant calls the tool
- minimal inbound/outbound audit logs

It does not parse `low`, `approve`, `status`, `summary`, or normal chat text by itself. Authorized messages go to the configured VibeCodingAssistant chat profile, which either replies naturally or calls a workflow tool.
...
VibeCodingAssistant can call these tools:
...
If a tool is not valid for the current workflow state, the tool layer returns the error to VibeCodingAssistant so it can explain in normal language.
