...
这个项目仍在早期阶段，所以可能会有 bug。  
The project is still at an early stage, so bugs may remain.

如果在使用过程中遇到 bug，欢迎提交 issue。  
If you encounter a bug, please open an issue.

维护者会尽快处理。  
Maintainers will review it as soon as possible.

## VibeCodingAssistant 使用说明

## VibeCodingAssistant Usage Instructions

VibeCodingAssistant 是一个面向 Web Coding 的 AI 工作流协调系统。  
VibeCodingAssistant is an AI workflow orchestration system for web coding.
...
- **VibeCodingAssistant**：负责和用户对话、创建任务、推进 workflow、展示 artifacts、记录状态和最终 task record。  
- **VibeCodingAssistant**: Talks with the user, creates tasks, advances the workflow, displays artifacts, records state, and generates the final task record.
...
一种可用的组合是 **DeepSeek API + Claude API + Codex API**。  
One supported setup combines **DeepSeek API + Claude API + Codex API**.

其中，VibeCodingAssistant 这个系统本身的对话和 workflow 调度层使用的是 DeepSeek API。  
The conversation and workflow orchestration layer of VibeCodingAssistant itself uses the DeepSeek API.
...
推荐的用法是：先用 ChatGPT 把任务 prompt 打磨清楚，再把整理好的 prompt 发给 VibeCodingAssistant 创建任务。  
A recommended workflow is to polish the task prompt with ChatGPT, then send the refined prompt to VibeCodingAssistant to create the task.

如果已经订阅 ChatGPT Plus，可以在现有订阅范围内使用 ChatGPT 打磨 prompt。  
If ChatGPT Plus is already available, prompt refinement can stay within that existing subscription.
...
- **DeepSeek** 负责 VibeCodingAssistant 的日常对话和 workflow 调度。  
- **DeepSeek** handles daily conversation and workflow orchestration for VibeCodingAssistant.
...
一个实用的 profile 配置是：  
One practical profile configuration is:

- **Prompt 打磨**：ChatGPT Plus。  
- **Prompt polishing**: ChatGPT Plus.

- **VibeCodingAssistant 对话层**：DeepSeek `deepseek-v4-flash`。  
- **VibeCodingAssistant conversation layer**: DeepSeek `deepseek-v4-flash`.
...
这个模式在成本和质量之间比较平衡，适合作为默认模式。  
This mode provides a good balance between cost and quality and works well as the default.
...
真正的 workflow 状态、任务记录、agent 调度和 artifacts 都由本地的 VibeCodingAssistant 维护。  
The actual workflow state, task records, agent orchestration, and artifacts are maintained locally by VibeCodingAssistant.

## 持续运行方式

## Continuous Running Mode

VibeCodingAssistant 本质上是一个运行在你本地电脑上的 assistant service。  
VibeCodingAssistant is essentially an assistant service running on your local computer.

只要你的电脑不关机，并且 VibeCodingAssistant 服务保持运行，它就可以一直通过 Lark 接收消息、创建任务、推进 workflow、调用 Claude / Codex 执行不同阶段的工作，并处理各种 coding task。  
As long as your computer stays on and the VibeCodingAssistant service keeps running, it can continuously receive messages through Lark, create tasks, advance workflows, call Claude / Codex for different stages, and handle coding tasks.
...
推荐做法是：**创建任务时，直接使用提前写好的高质量 prompt**。  
Recommended workflow: **create tasks with a high-quality prompt prepared in advance**.

通常可以先在 ChatGPT 里把需求打磨清楚，再把最终 prompt 发给 VibeCodingAssistant 创建任务。  
A typical flow is to polish the requirement in ChatGPT, then send the final prompt to VibeCodingAssistant to create the task.
...
用户一般只需要通过 Lark 和 VibeCodingAssistant 对话即可。  
The user usually only needs to talk with VibeCodingAssistant through Lark.
...
2. 在 Lark 里把整理好的 prompt 发给 VibeCodingAssistant，创建 coding task。  
2. Send the refined prompt to VibeCodingAssistant in Lark to create a coding task.
