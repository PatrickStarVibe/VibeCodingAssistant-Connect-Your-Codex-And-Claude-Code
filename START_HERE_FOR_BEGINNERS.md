# VibeCodingAssistant Beginner Setup Guide

This guide is for users who are new to command-line setup. It explains what to edit, what each file means, and how to check that VibeCodingAssistant can see your own project.
...
VibeCodingAssistant is the local workflow repo for VibeCodingAssistant. It helps you run a structured AI coding workflow around your own project:

- You tell VibeCodingAssistant what you want changed.
- VibeCodingAssistant creates a task record and routes planning, implementation, review, and final acceptance.
- VibeCodingAssistant can send prompts to your configured LLM provider or local coding-agent command.
- VibeCodingAssistant can optionally connect to Lark / Feishu so you can drive the workflow from chat.

VibeCodingAssistant is not the project you want to edit. VibeCodingAssistant needs to know the path to that project, then it can pass the right workspace and project-memory files to the agents.
...
- This VibeCodingAssistant repo downloaded locally.
- The local path of the project you want VibeCodingAssistant to operate on, for example `<YOUR_PROJECT_PATH>`.
...
## Open A Terminal In The VibeCodingAssistant Folder

On Windows, open File Explorer, go to the VibeCodingAssistant folder, click the address bar, type `powershell`, then press Enter.

You should now see a PowerShell window opened in the VibeCodingAssistant folder. All commands below assume that window is already in the VibeCodingAssistant folder.
...
If either command says it is not recognized, install Node.js, close PowerShell, open it again in the VibeCodingAssistant folder, and retry the two commands.
...
VibeCodingAssistant keeps shareable examples separate from your private local setup:

- `.env.example` is the safe example for environment variables.
- `assistant.config.example.json` is the safe example for VibeCodingAssistant configuration.
- `.env.local` is your private secret file.
- `assistant.config.local.json` is your private local VibeCodingAssistant config.
...
If PowerShell asks whether to overwrite a file, stop and check whether you already configured VibeCodingAssistant before replacing it.
...
If your provider works through a local command instead of an API key, keep any secrets required by that command in `.env.local` or your system environment. Put only the command name or command path in the VibeCodingAssistant profile.

## Fill In Your Project Path

Open `assistant.config.local.json`. The most important project fields are:

- `workspace.targetDir`: the default project folder VibeCodingAssistant should use.
- `defaultProjectId`: the project id VibeCodingAssistant uses when no project is specified.
- `projects[]`: the list of projects VibeCodingAssistant can operate on.
- `projects[].targetDir`: the real local path of one project.
- `projects[].docsDir`: the project-memory folder for VibeCodingAssistant to read.
...
- `<YOUR_PROJECT_PATH>` with the folder path of the project you want VibeCodingAssistant to edit.
...
In `assistant.config.local.json`, `profiles` describes how VibeCodingAssistant reaches each LLM provider or coding agent. `workflowRoles` tells VibeCodingAssistant which profile to use for each role.
...
## Verify VibeCodingAssistant Can Read Your Project
...
You want to see your `<PROJECT_ID>` and `<YOUR_PROJECT_PATH>` in the output. If you do, VibeCodingAssistant can load the project entry.

Next create a tiny smoke task:

```powershell
npm run assistant -- create --config assistant.config.local.json --project <PROJECT_ID> --title "setup-smoke-test" --task "Check whether this VibeCodingAssistant setup can read the configured project root and project docs."
```
...
In the preview, check that `Target workspace` points to your project path, not the VibeCodingAssistant repo path unless VibeCodingAssistant itself is the project you are testing.
...
Use this only if you want to control VibeCodingAssistant from Lark / Feishu.
...
`config missing` or VibeCodingAssistant cannot find `assistant.config.local.json`:
