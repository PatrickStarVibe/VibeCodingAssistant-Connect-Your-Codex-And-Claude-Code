...
export class WorkflowService {
  ...
  async finalReview(taskIdOrLatest: string, _preGit?: GitSnapshot, _postGit?: GitSnapshot): Promise<WorkflowResult> {
    ...
    state = await this.store.appendArtifact(state, 'decision-log', 'final review routing requested user direction without structured options; VibeCodingAssistant generated fallback options');
    await this.store.saveState(state);
    return { state, message: state.pendingUserPrompt ?? 'Final review needs a user direction decision.' };
  }
...
function renderPlanArtifactFailurePrompt(input: {
  ...
  const source = input.plan?.sourcePath ?? input.error?.sourcePath ?? 'stdout';
  return [
    'Heavy agent did not provide a usable plan artifact. VibeCodingAssistant paused the workflow instead of sending an empty plan to Reviewer/Planner.',
    `Phase: ${input.phase}`,
    `Source: ${source}`,
    `Reason: ${input.reason}`,
    '',
...
function makePlanArtifactFailureDecision(input: {
  ...
  return {
    ...
    question: `The heavy agent did not provide a usable plan artifact during ${input.phase}. What should VibeCodingAssistant do next?`,
    rationale: `VibeCodingAssistant paused before sending an empty or unusable plan onward. Source: ${source}. Reason: ${input.reason}`,
    options: [
      {
        id: 'A',
...
function makeFinalReviewFollowupCapDecision(state: TaskState, reason: string): PendingUserDecision {
  ...
  return {
    ...
    source: 'final_review',
    question: 'Final review still has implementation blockers after one follow-up. What should VibeCodingAssistant do next?',
    rationale: [
      'VibeCodingAssistant already ran one scoped Final Review Follow-up.',
...
function makeFinalReviewRoutingFallbackDecision(reason: string, userPrompt?: string): PendingUserDecision {
  ...
  return {
    ...
    source: 'final_review',
    question: 'Final review needs a next-step decision. What should VibeCodingAssistant do?',
    rationale: [
      'The final-review routing step asked for user direction but did not provide structured options.',
      'VibeCodingAssistant generated safe fallback options instead of exposing the internal formatting failure.',
      '',
      'Latest final review reason:',
      trimmed,
